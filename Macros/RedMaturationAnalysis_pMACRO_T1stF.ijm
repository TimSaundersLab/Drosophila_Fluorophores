...
function processFile(input, output, file) {
	...
	run("Bio-Formats Importer", "open=[" + input + File.separator + file + "] color_mode=Default rois_import=[ROI manager] specify_range view=Hyperstack stack_order=XYCZT c_begin=1 c_end=1 c_step=1 z_begin=2 z_end=7 z_step=1 t_begin=" + True1stFrame + " t_end=" + frames + " t_step=1");
	...
	run("Analyze Particles...", "size=12-51.00 circularity=0.40-1.00 display exclude clear include summarize add stack");
